...
# %% auto 0
__all__ = ['root', 'logger', 'convert_size', 'parallel_df_function', 'parallel_apply', 'chunk_df', 'initialize_chunk_dir',
           'parallel_apply_chunks', 'compile_chunks']
...
def parallel_df_function(df, func, npartitions=int(0.9 * mp.cpu_count()), keep_index=False):
    ...
    logger.info(f"DF size: {convert_size(memory_usage)}")
    ...
    df.index = og_index
    if result.shape[0] == df.shape[0]: result.index = og_index
...
def parallel_apply(df, func, meta, npartitions=int(0.9 * mp.cpu_count()), keep_index=False, **kargs):
    ...
    if isinstance(df, pd.DataFrame):
        def apply_func(ddf): return ddf.apply(func, meta=meta, axis=1, **kargs).compute()
    else:
        def apply_func(ddf): return ddf.apply(func, meta=meta, **kargs).compute()
    
    return parallel_df_function(
        df, apply_func, npartitions, keep_index
    )
